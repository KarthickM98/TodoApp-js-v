...
function page(){ //page switching logic
...
function createTask(e){
    ...
    taskLi.push(task); // adding tasks 
    showList();// adding the card in main page    
}//---------------------------------------------------------------------
function showList(){ //to show the task cards
    var listitem=` `
    ...
    if(taskLi.length!=0){   // removing the no item text
...
// adding an array in an object;
function addTask(id){   /// want to pass this id to the create task function??
    //so create nested functon  and nested function caused error so used arrow function
    taskLi.forEach(element=>{
        if(element.id===id){
            ...
            addBtnTwo.onclick=()=>{ // add button arrow function that adds task to the inner array   
...
function goBack(){ // back button function
    pgSet=true;
    page();
}//---------------------------------------------------------------------
